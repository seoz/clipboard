...
**A decrypt failure never advances the pull cursor.** The delta query is `updatedAt > cursor`; advancing it past a document this device couldn't read would make that failure permanent rather than retryable — the entry would simply vanish from every future pull, even after the correct key became available. A document that fails to decrypt is retried on every subsequent pull instead, at the cost of one extra re-fetch per cycle until it resolves.

**A brand-new entry that fails to decrypt is not silently dropped.** It materializes locally as a placeholder — `text: null, undecryptable: true` — rendered as "Could not decrypt this snippet" with copy, edit and delete all refused. Delete in particular has to be refused rather than merely disabled: tombstoning something this device cannot verify would delete it for every device, including ones that decrypt it fine. The same guard exists one layer down, in `push()` itself — a placeholder can never be uploaded, because encrypting its `null` text would overwrite real content on the server with garbage. That guard is unconditional, independent of how an id might end up queued.

### 8.5 Passphrase lifecycle and account reset (built)

**Changing the passphrase** does not decrypt-then-re-encrypt anything, because local storage was never encrypted in the first place — only the server copy is. Rotation derives a new key, rotates the account's `kdf`/`verifier` first, then bumps every local entry's clock (skipping any undecryptable placeholder, for the same reason `push()` refuses one) and lets the ordinary push pipeline re-upload everything under whichever key is now cached. That reuse is deliberate: rotation gets the same batching, backoff and placeholder guard as any other push, rather than a second, separately-tested path for the highest-blast-radius operation in the app. Requires the device to already be unlocked — not because the old key is needed, but because unlocked is the only available proof that whoever is rotating the passphrase actually knows the current one, rather than merely being signed into the Google account. Propagation is not instant: a device still holding the old key simply stops receiving updates for entries encrypted under the new one until someone enters the new passphrase there too — surfaced to the user as expected behaviour, not left to be discovered as an apparent sync failure.

**Deleting all cloud data** (danger zone) removes every entry from the server and forgets the account's `kdf`/`verifier`, without touching local snippets or the Google session — a narrower reset than signing out entirely. The security rules only permit a hard delete of an already-tombstoned document, so this runs in two passes: every live entry is tombstoned first, then removed. The first version of this skipped that and hard-deleted live entries directly — it passed every unit test, because the mocked Firestore doesn't enforce any rules, and was only caught by running the same operation against the real emulator. That gap is why `test/rules.test.js` exists at all: rules correctness cannot be verified by the application's own mocks, only against the real rule evaluator.
...
- The 20-item cap applies only to local-only (signed-out) use; a signed-in device is uncapped (`maxTexts` returns `Infinity` once sync is enabled).
...
- The two-device merge and pull paths are covered by unit tests against a faithful mock of Firestore's query semantics, and the security rules are covered against the real emulator, but neither substitutes for an actual two-profile, two-account run — that verification needs a human at two real devices and is recorded as the acceptance step for sync rather than something automatable here.
- An undecryptable placeholder that a device can never resolve (a permanently different key, say) has no user-facing cleanup path today beyond a full account reset via the danger zone; it simply stays a broken row.
